...
{
    public partial class 商户账号密码支付 : System.Web.UI.Page
    {
        ...
        protected void Button_更新密码_Click(object sender, EventArgs e)
        {
            ...
            if (TextBox_商户旧支付密码.Text.Length > 0 && TextBox_商户新支付密码.Text.Length > 0 && TextBox_商户新支付密码再次确认.Text.Length > 0)
            {
                if (TextBox_商户新支付密码.Text == TextBox_商户新支付密码再次确认.Text)
                {
        ...
        private void 操作更新()
        {
            ...
            using (MySqlConnection con = new MySqlConnection(ClassLibrary1.ClassDataControl.conStr1))
            {
                using (MySqlCommand cmd = new MySqlCommand("SELECT 商户ID,支付密码 FROM table_商户账号 WHERE 商户ID=@商户ID", con))
                {
                    cmd.Parameters.AddWithValue("@商户ID", Cookie_UserName);
                    using (MySqlDataAdapter da = new MySqlDataAdapter(cmd))
                    {
                        ...
                        {
                            this.Label_商户ID.Text = dr["商户ID"].ToString();
                            string 支付密码旧 = dr["支付密码"].ToString();

                            if (支付密码旧 == TextBox_商户旧支付密码.Text)
                            {

                                using (MySqlConnection con12 = new MySqlConnection(ClassLibrary1.ClassDataControl.conStr1))
                                {
                                    using (MySqlCommand cmd12 = new MySqlCommand("UPDATE table_商户账号 SET 支付密码=@支付密码 WHERE 商户ID=@商户ID ", con12))
                                    {
                                        cmd12.Parameters.AddWithValue("@商户ID", Cookie_UserName);
                                        cmd12.Parameters.AddWithValue("@支付密码", TextBox_商户新支付密码.Text);
