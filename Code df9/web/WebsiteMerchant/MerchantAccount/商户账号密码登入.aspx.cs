...
{
    public partial class 商户账号密码登入 : System.Web.UI.Page
    {
        ...
        private void 操作更新()
        {
            ...
            using (MySqlConnection con = new MySqlConnection(ClassLibrary1.ClassDataControl.conStr1))
            {
                using (MySqlCommand cmd = new MySqlCommand("SELECT 商户ID,商户密码 FROM table_商户账号 WHERE 商户ID=@商户ID", con))
                {
                    cmd.Parameters.AddWithValue("@商户ID", Cookie_UserName);
                    using (MySqlDataAdapter da = new MySqlDataAdapter(cmd))
                    {
                        ...
                        foreach (DataRow dr in images.Rows)
                        {
                            ...
                            if (商户密码旧 == TextBox_商户旧登入密码.Text)
                            {
                                ...
                                {
                                    using (MySqlCommand cmd12 = new MySqlCommand("UPDATE table_商户账号 SET 商户密码=@商户密码 WHERE 商户ID=@商户ID ", con12))
                                    {
                                        cmd12.Parameters.AddWithValue("@商户ID", Cookie_UserName);
                                        cmd12.Parameters.AddWithValue("@商户密码", TextBox_商户新登入密码.Text);
