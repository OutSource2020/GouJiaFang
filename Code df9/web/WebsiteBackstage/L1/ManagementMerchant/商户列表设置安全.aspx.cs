...
{
    public partial class 商户列表设置安全 : System.Web.UI.Page
    {
      ...
      using (var db = (new DBClient()).GetClient())
      {
        ...
        protected void Button_设置账号密码和支付密码_Click(object sender, EventArgs e)
        {
            string 编号 = 从URL获取值();
            Response.Redirect("商户列表设置安全密码.aspx?Bianhao=" + 编号 + "");
        }

        protected void Button_设置绑定邮箱_Click(object sender, EventArgs e)
        {
            string 编号 = 从URL获取值();
            Response.Redirect("商户列表设置安全绑定邮箱.aspx?Bianhao=" + 编号 + "");
        }

        protected void Button_设置绑定手机_Click(object sender, EventArgs e)
        {
            string 编号 = 从URL获取值();
            Response.Redirect("商户列表设置安全绑定手机.aspx?Bianhao=" + 编号 + "");
        }
        ...
        protected void Button_清空支付错误累计_Click(object sender, EventArgs e)
        {
            string 从URL传来值 = 从URL获取值();

            using (MySqlConnection con = new MySqlConnection(ClassLibrary1.ClassDataControl.conStr1))
            {
                using (MySqlCommand cmd = new MySqlCommand("UPDATE table_商户账号 SET 支付错误累计='0' WHERE 商户ID=@商户ID ", con))
                {
                    cmd.Parameters.AddWithValue("@商户ID", 从URL传来值);
                    ...
                    ClassLibrary1.ClassMessage.HinXi(Page, "支付错误累计清零成功!");
                    //Response.Redirect(Request.Url.AbsoluteUri, false);
                }
            }
        }
        ...
        {
            string 编号 = 从URL获取值();
            Response.Redirect("商户列表设置安全QR.aspx?Bianhao=" + 编号 + "");
        }
