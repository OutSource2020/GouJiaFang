...
{
    public partial class 商户列表设置安全绑定手机 : System.Web.UI.Page
    {
        ...
        private void GetCustomer()//获得数据
        {
            ...
            {
                using (MySqlCommand cmd = new MySqlCommand("SELECT 商户ID,绑定手机 FROM table_商户账号 WHERE 商户ID=@商户ID", con))
                {
        ...
        private void 操作更新()
        {
            ...
            using (MySqlConnection con = new MySqlConnection(ClassLibrary1.ClassDataControl.conStr1))
            {
                using (MySqlCommand cmd = new MySqlCommand("UPDATE table_商户账号 SET 绑定手机=@绑定手机 WHERE 商户ID=@商户ID ", con))
                {
