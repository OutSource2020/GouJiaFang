...
{
    public partial class 商户列表 : System.Web.UI.Page
    {
        public static string 时间字段 = "时间注册";
        ...
        //    {
        //        using (MySqlCommand cmd = new MySqlCommand("SELECT 商户ID,商户名称,提款余额,手续费余额,时间注册,最后登入时间,所属管理员,状态 FROM table_商户", con))
        //        {
        ...
        protected void RadioButton_状态启用_CheckedChanged(object sender, EventArgs e)
        {
            BindGrid("where " + 查看勾选了哪些() + " ");
        }

        protected void RadioButton_状态停用_CheckedChanged(object sender, EventArgs e)
        {
            BindGrid("where " + 查看勾选了哪些() + " ");
        }
        ...
        private void BindGrid(string 时间导入绑定)
        {
            using (MySqlConnection connC = new MySqlConnection(ClassLibrary1.ClassDataControl.conStr1))
            {
                ...
                MySqlCommand cmd1 = new MySqlCommand("select count(商户ID) from table_商户账号 " + 时间导入绑定 + " order by 时间注册 desc", connC);
                object obj1 = cmd1.ExecuteScalar();
                if (obj1 != null)
                {
                    Label_商户总数.Text = obj1.ToString();
                }

                MySqlCommand cmd2 = new MySqlCommand("select sum(提款余额) from table_商户账号 " + 时间导入绑定 + " order by 时间注册 desc", connC);
                object obj2 = cmd2.ExecuteScalar();
                if (obj2 != null)
                {
                    Label_提款总余额.Text = obj2.ToString();
                }

                MySqlCommand cmd3 = new MySqlCommand("select sum(手续费余额) from table_商户账号 " + 时间导入绑定 + " order by 时间注册 desc", connC);
                object obj3 = cmd3.ExecuteScalar();
                if (obj3 != null)
                {
                    Label_手续费总余额.Text = obj3.ToString();
                }

                connC.Close();
            }

            string strQuery = "SELECT 商户ID,商户名称,提款余额,手续费余额,手续费比率,所属管理L2,所属代理L1,所属代理L2,状态,时间最后登入,时间注册 FROM table_商户账号  " + 时间导入绑定 + " order by 时间注册 desc";
        ...
        public string 获得订单状态()
        {
            ...
            if (RadioButton_状态启用.Checked)
            {
                条件2 = "  状态='启用' ";
            }
            if (RadioButton_状态停用.Checked)
            {
                条件2 = "  状态='停用' ";
            }
        ...
        public string 获得筛选关键字()
        {
            ...
            string redkey = DropDownList_筛选关键字.SelectedItem.Value;
            if (redkey == "未选择")
            {
            ...
            {
                条件5 = " " + DropDownList_筛选关键字.SelectedItem.Value + "='" + TextBox_筛选关键字.Text + "' ";
                中间加和5 = " and ";
            }
