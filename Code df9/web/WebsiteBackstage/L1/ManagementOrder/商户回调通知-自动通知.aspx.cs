...
{
    public partial class 商户回调通知_自动通知 : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                {
                    var isOpen = db.Queryable<table_config>().Where(it => it.comment == "自动通知").Select(it => it.value).First();
                    Button_自动通知.Text = "自动通知（" + (int.Parse(isOpen) == 1 ? "开启" : "关闭") + "）";
                }
            }
        }
        ...
        protected void Button_自动通知_Click(object sender, EventArgs e)
        {
            using (SqlSugarClient db = new DBClient().GetClient())
            {
                var isOpen = db.Queryable<table_config>().Where(it => it.comment == "自动通知").Select(it => it.value).First();
                isOpen = int.Parse(isOpen) == 1 ? "0" : "1";
                db.Updateable<table_config>().Where(it => it.comment == "自动通知").SetColumns(it => it.value == isOpen).ExecuteCommand();
                Button_自动通知.Text = "自动通知（" + (int.Parse(isOpen) == 1 ? "开启" : "关闭") + "）";
            }
        }
    }
