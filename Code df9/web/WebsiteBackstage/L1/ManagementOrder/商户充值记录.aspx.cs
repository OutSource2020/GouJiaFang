...
namespace web1.WebsiteBackstage.L1.ManagementOrder
{
    public partial class 商户充值记录 : System.Web.UI.Page
    {
        ...
        {
            return  获得类型() + 获得筛选关键字() + 获得区间金额() + 获得时间();
        }
        ...
        private void BindGrid(string 时间导入绑定)
        {
            ...
            string strQuery = "select 订单号,商户ID,商户银行卡卡号,商户充值目标卡号,充值类型,充值金额,备注后台,状态,时间创建,商户充值目标姓名 from table_商户明细充值 where " + 时间导入绑定 + " order by 时间创建 desc  LIMIT " + 分页() + " ";
