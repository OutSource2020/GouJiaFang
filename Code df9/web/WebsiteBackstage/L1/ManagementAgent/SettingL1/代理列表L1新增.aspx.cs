...
{
    public partial class 代理列表L1新增 : System.Web.UI.Page
    {
        ...
        private bool 判断是否存在相同ID()
        {
            ...
            {
                using (MySqlCommand cmd = new MySqlCommand("select * from table_代理账号等级1 where 代理ID=@代理ID", con))
                {
        ...
        private void 新增()
        {
            if (判断是否存在相同ID() == false)
            {
                ...
                using (MySqlConnection scon = new MySqlConnection(sql))
                {
                    ...
                    string str = "insert into table_代理账号等级1(代理ID,代理名称,时间注册,状态,登入错误累计,keyga) values(@代理ID,@代理名称,@时间注册,@状态,@登入错误累计,@keyga)";
