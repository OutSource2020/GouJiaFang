...
{
    public partial class 代理列表L1设置安全绑定手机 : System.Web.UI.Page
    {
        ...
        private void GetCustomer()//获得数据
        {
            ...
            {
                using (MySqlCommand cmd = new MySqlCommand("SELECT 代理ID,绑定手机 FROM table_代理账号等级1 WHERE 代理ID=@代理ID", con))
                {
        ...
        private void 操作更新()
        {
            ...
            using (MySqlConnection con = new MySqlConnection(ClassLibrary1.ClassDataControl.conStr1))
            {
                using (MySqlCommand cmd = new MySqlCommand("UPDATE table_代理账号等级1 SET 绑定手机=@绑定手机 WHERE 代理ID=@代理ID ", con))
                {
