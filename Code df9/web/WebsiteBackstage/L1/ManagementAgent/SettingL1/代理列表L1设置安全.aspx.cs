...
{
    public partial class 代理列表L1设置安全 : System.Web.UI.Page
    {
        ...
        private void GetCustomer2()//获得数据-安全
        {
            ...
            {
                using (MySqlCommand cmd = new MySqlCommand("SELECT 代理ID,绑定邮箱,绑定手机,登入错误累计 FROM table_代理账号等级1 WHERE 代理ID=@代理ID", con))
                {
        ...
        {
            string 编号 = 从URL获取值();
            Response.Redirect("代理列表L1设置安全密码.aspx?Bianhao=" + 编号 + "");
        }

        protected void Button_设置绑定邮箱_Click(object sender, EventArgs e)
        {
            string 编号 = 从URL获取值();
            Response.Redirect("代理列表L1设置安全绑定邮箱.aspx?Bianhao=" + 编号 + "");
        }

        protected void Button_设置绑定手机_Click(object sender, EventArgs e)
        {
            string 编号 = 从URL获取值();
            Response.Redirect("代理列表L1设置安全绑定手机.aspx?Bianhao=" + 编号 + "");
        }
        ...
        protected void Button_清空登入错误累计_Click(object sender, EventArgs e)
        {
            ...
            using (MySqlConnection con = new MySqlConnection(ClassLibrary1.ClassDataControl.conStr1))
            {
                using (MySqlCommand cmd = new MySqlCommand("UPDATE table_代理账号等级1 SET 登入错误累计='0' WHERE 代理ID=@代理ID ", con))
                {
                    cmd.Parameters.AddWithValue("@代理ID", 从URL传来值);
        ...
        {
            string 编号 = 从URL获取值();
            Response.Redirect("代理列表L1设置安全QR.aspx?Bianhao=" + 编号 + "");
        }

        protected void Button_返回_Click(object sender, EventArgs e)
        {
            string 编号 = 从URL获取值();
            Response.Redirect("代理列表L1设置.aspx?Bianhao=" + 编号 + "");
        }
    }
}