...
{
    public partial class 代理列表L1设置信息 : System.Web.UI.Page
    {
        ...
        private void GetCustomer()//获得数据
        {
            ...
            {
                using (MySqlCommand cmd = new MySqlCommand("SELECT 代理ID,代理名称,状态,时间最后登入,时间注册 FROM table_代理账号等级1 WHERE 代理ID=@代理ID", con))
                {
        ...
        private void 操作更新()
        {
            ...
            using (MySqlConnection con = new MySqlConnection(ClassLibrary1.ClassDataControl.conStr1))
            {
                using (MySqlCommand cmd = new MySqlCommand("UPDATE table_代理账号等级1 SET 代理名称=@代理名称 , 状态=@状态   WHERE 代理ID=@代理ID ", con))
                {
