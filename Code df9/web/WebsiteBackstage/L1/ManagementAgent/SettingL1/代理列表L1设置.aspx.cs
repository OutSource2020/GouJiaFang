...
{
    public partial class 代理列表L1设置 : System.Web.UI.Page
    {
        ...
        private void GetCustomer()//获得数据
        {
            ...
            {
                string 查哪些 = "代理ID,代理名称,状态,时间最后登入,时间注册";
                string 哪个表 = " table_代理账号等级1 ";
        ...
        private void 获得代理安全列信息()//获得数据-安全
        {
            ...
            using (MySqlConnection con = new MySqlConnection(ClassLibrary1.ClassDataControl.conStr1))
            {
                using (MySqlCommand cmd = new MySqlCommand("SELECT 绑定邮箱,绑定手机,登入错误累计 FROM table_代理账号等级1 WHERE 代理ID=@代理ID", con))
                {
                    cmd.Parameters.AddWithValue("@代理ID", 从URL传来值);
                    using (MySqlDataAdapter da = new MySqlDataAdapter(cmd))
                    {
        ...
        {
            string 编号= 从URL获取值();
            Response.Redirect("代理列表L1设置信息.aspx?Bianhao="+ 编号 + "");
        }
