...
{
    public partial class 代理列表L1 : System.Web.UI.Page
    {
        ...
        {
            Response.Redirect("代理列表L1新增.aspx");
        }

        protected void Button_查找_Click(object sender, EventArgs e)
        {
            if (ClassLibrary1.ClassYZ.检查日期格式yyyyMMdd(TextBox_开始时间.Text)|| ClassLibrary1.ClassYZ.检查日期格式yyyyMMdd(TextBox_结束时间.Text) == true )
            {
                BindGrid("where " + 查看勾选了哪些() + " ");
            }
            else
            {
                ClassLibrary1.ClassMessage.HinXi(Page, "时间格式不符合要求");
            }
        ...
        {
            string strQuery = "SELECT 代理ID,代理名称,时间最后登入,时间注册,状态 FROM table_代理账号等级1 " + 时间导入绑定 + " order by 时间注册 desc";
