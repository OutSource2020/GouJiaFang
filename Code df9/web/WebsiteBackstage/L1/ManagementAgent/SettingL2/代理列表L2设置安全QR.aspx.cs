...
{
    public partial class 代理列表L2设置安全QR : System.Web.UI.Page
    {
        ...
        protected void Button_验证_Click(object sender, EventArgs e)
        {
            ...
            {
                using (MySqlCommand cmd = new MySqlCommand("SELECT 代理ID,keyga FROM table_代理账号等级2 WHERE 代理ID=@代理ID", con))
                {
        ...
        protected void Button_重置生成新的_Click(object sender, EventArgs e)
        {
            ...
            using (MySqlConnection con = new MySqlConnection(ClassLibrary1.ClassDataControl.conStr1))
            {
                using (MySqlCommand cmd = new MySqlCommand("UPDATE table_代理账号等级2 SET keyga=@keyga WHERE 代理ID=@代理ID ", con))
                {
