...
{
    public partial class 代理列表L2设置安全密码 : System.Web.UI.Page
    {
        ...
        private void GetCustomer()//获得数据
        {
            ...
            {
                using (MySqlCommand cmd = new MySqlCommand("SELECT 代理ID,代理密码 FROM table_代理账号等级2 WHERE 代理ID=@代理ID", con))
                {
        ...
        private void 更新开始()
        {
            ...
            using (MySqlConnection con = new MySqlConnection(ClassLibrary1.ClassDataControl.conStr1))
            {
                using (MySqlCommand cmd = new MySqlCommand("SELECT 代理ID,绑定邮箱 FROM table_代理账号等级2 WHERE 代理ID=@代理ID ", con))
                {
                    cmd.Parameters.AddWithValue("@代理ID", 从URL传来值);
                    ...
                    using (MySqlDataAdapter da = new MySqlDataAdapter(cmd))
                    {
                        ...
                        foreach (DataRow dr in images.Rows)
                        {
                            ...
                            {
                                using (MySqlCommand cmd12 = new MySqlCommand("UPDATE table_代理账号等级2 SET 代理密码=@代理密码 WHERE 代理ID=@代理ID ", con12))
                                {
