...
{
    public partial class 管理收款银行卡 : System.Web.UI.Page
    {
        ...
        {
            Response.Redirect("./管理收款银行卡卡新增.aspx");
        }
        ...
        public string 获得筛选端金额()
        {
            ...
            else
            {
                ...
                {
                    下拉表内容 = " 收款银行卡余额<='" + TextBox_筛选端金额.Text + "'";
                }
        ...
        {
            string strQuery = "select 编号,收款银行卡名称,收款银行卡卡号,收款银行名称,收款银行卡余额,收款银行卡主姓名,收款银行卡主电话,显示标记,状态,时间创建,手续卡,金额卡 from table_后台收款银行卡管理 " + 时间导入绑定 + " order by 时间创建 desc";
