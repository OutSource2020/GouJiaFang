...
{
    public partial class 管理出款银行卡流水 : System.Web.UI.Page
    {
        ...
        private void 下拉框选银行卡()
        {
            ...
            DropDownList_银行卡.DataTextField = ds.Tables["table_后台出款银行卡管理"].Columns["出款银行卡名称"].ToString();
            DropDownList_银行卡.DataValueField = ds.Tables["table_后台出款银行卡管理"].Columns["出款银行卡卡号"].ToString();
        ...
        public string 获得筛选银行卡()
        {
            ...
            {
                条件5 = " 出款银行卡卡号='" + DropDownList_银行卡.SelectedItem.Value + "' ";
                中间加和5 = " and ";
            }
        ...
        {
            string strQuery = "select 订单号,收入,支出,余额,商户ID,出款银行卡卡号,出款银行卡名称,备注,类型,状态,时间创建,时间交易 from table_后台出款银行卡流水 where  " + 时间导入绑定 + " order by 时间交易 desc  LIMIT " + 分页() + " ";
            ...
            using (MySqlConnection connC = new MySqlConnection(ClassLibrary1.ClassDataControl.conStr1))
            {
                ...
                if (obj14 != null)
                {
                    Label_其他金额.Text = obj14.ToString();
                }
