...
{
    public partial class 管理收款银行卡流水 : System.Web.UI.Page
    {
        public static string 时间字段 = "时间交易";
        ...
        private void 下拉框选商户()
        {
            ...
            myadapter.Fill(ds, "table_商户");
            myconn.Close();
            DropDownList_商户ID.DataSource = ds.Tables[0].DefaultView;
            DropDownList_商户ID.DataTextField = ds.Tables["table_商户"].Columns["商户名称"].ToString();
            DropDownList_商户ID.DataValueField = ds.Tables["table_商户"].Columns["商户ID"].ToString();
            DropDownList_商户ID.DataBind();
            DropDownList_商户ID.Items.Insert(0, new ListItem("未选择", "未选择"));

            myconn.Close();
        }

        private void 下拉框选银行卡()
        {
            string connstring = ClassLibrary1.ClassDataControl.conStr1;
            string querystring = "select distinct 收款银行卡名称,收款银行卡卡号 from table_后台收款银行卡管理 where 状态 = '启用'";
            MySqlConnection myconn = new MySqlConnection(connstring);
            myconn.Open();
            MySqlDataAdapter myadapter = new MySqlDataAdapter(querystring, myconn);
            DataSet ds = new DataSet();
            ...
            myconn.Close();
            DropDownList_银行卡.DataSource = ds.Tables[0].DefaultView;
            DropDownList_银行卡.DataTextField = ds.Tables["table_后台收款银行卡管理"].Columns["收款银行卡名称"].ToString();
            DropDownList_银行卡.DataValueField = ds.Tables["table_后台收款银行卡管理"].Columns["收款银行卡卡号"].ToString();
            DropDownList_银行卡.DataBind();
            DropDownList_银行卡.Items.Insert(0, new ListItem("未选择", "未选择"));

            myconn.Close();
        }

        public string 获得筛选类型()
        {
            string 中间加和5 = "";
            string 条件5 = "";


            string redkey = DropDownList_类型.SelectedItem.Value;
            if (redkey == "全部")
            {
            ...
            {
                条件5 = " 类型='" + DropDownList_类型.SelectedItem.Value + "' ";
                中间加和5 = " and ";
            }
        ...
        public string 获得筛选状态()
        {
            string 中间加和5 = "";
            string 条件5 = "";


            string redkey = DropDownList_状态.SelectedItem.Value;
            if (redkey == "全部")
            {
            ...
            {
                条件5 = " 状态='" + DropDownList_状态.SelectedItem.Value + "' ";
                中间加和5 = " and ";
            }
        ...
        public string 获得筛选商户ID()
        {
            string 中间加和5 = "";
            string 条件5 = "";

            string redkey = DropDownList_商户ID.SelectedItem.Value;
            if (redkey == "未选择")
            {
            ...
            {
                条件5 = " 商户ID='" + DropDownList_商户ID.SelectedItem.Value + "' ";
                中间加和5 = " and ";
            }
            return 条件5 + 中间加和5;
        }

        public string 获得筛选银行卡()
        {
            string 中间加和5 = "";
            string 条件5 = "";


            string redkey = DropDownList_银行卡.SelectedItem.Value;
            if (redkey == "未选择")
            {
            ...
            {
                条件5 = " 收款银行卡卡号='" + DropDownList_银行卡.SelectedItem.Value + "' ";
                中间加和5 = " and ";
            }
        ...
        {
            return  获得筛选类型() + 获得筛选状态() + 获得筛选商户ID() + 获得筛选银行卡() + 获得时间();
        }

        private void BindGrid(string 时间导入绑定)
        {
            string strQuery = "select 订单号,收入,支出,余额,商户ID,收款银行卡卡号,收款银行卡名称,备注,类型,状态,时间创建,时间交易 from table_后台收款银行卡流水 where  " + 时间导入绑定 + " order by 时间交易 desc  LIMIT " + 分页() + " ";
        ...
        //====================================================================================================
        protected void DropDownList_类型_SelectedIndexChanged(object sender, EventArgs e)
        {
            BindGrid(" " + 查看勾选了哪些() + " ");
        }

        protected void DropDownList_状态_SelectedIndexChanged(object sender, EventArgs e)
        {
            BindGrid(" " + 查看勾选了哪些() + " ");
        }

        protected void DropDownList_商户ID_SelectedIndexChanged(object sender, EventArgs e)
        {
            BindGrid(" " + 查看勾选了哪些() + " ");
        }

        protected void DropDownList_银行卡_SelectedIndexChanged(object sender, EventArgs e)
        {
            BindGrid(" " + 查看勾选了哪些() + " ");
        }
