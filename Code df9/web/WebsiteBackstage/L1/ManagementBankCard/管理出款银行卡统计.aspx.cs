...
{
    public partial class 管理出款银行卡统计 : System.Web.UI.Page
    {
        ...
        {
            string strQuery = "select count(收入),count(支出),sum(收入),sum(支出),出款银行卡卡号,出款银行卡名称 from table_后台出款银行卡流水 where  " + 时间导入绑定 + "  group by 出款银行卡卡号,出款银行卡名称";
