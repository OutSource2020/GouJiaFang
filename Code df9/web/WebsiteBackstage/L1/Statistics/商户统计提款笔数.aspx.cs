...
namespace WebApplicationSHNS.WebsiteBackstage.L1.Statistics
{
    public partial class 商户统计提款笔数 : System.Web.UI.Page
    {
        ...
        {
            string strQuery = "select A.商户ID,max(B.商户名称),count(A.手续费),sum(A.交易金额),sum(A.手续费) from table_商户明细提款 A left join table_商户账号 B ON A.商户ID = B.商户ID where  A.状态='成功'  and " + 时间导入绑定 + " group by A.商户ID  LIMIT " + 分页() + " ";
