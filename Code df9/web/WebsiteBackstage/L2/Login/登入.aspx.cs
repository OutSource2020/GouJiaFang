...
namespace web1.WebsiteBackstage.L2.Login
{
        ...
        protected void Button_登入_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                    ...
                    TextBox_输入员工号.Text.Length < 10)
                {
                    ...
                    {
                        using (MySqlCommand cmd11 = new MySqlCommand("SELECT 后台ID,keyga,状态 FROM table_后台账号 WHERE 后台ID=@后台ID and 后台账号分级='L2' ", con11))
                        {
                                            ...
                                            if (result)
                                            {
                                                ...
                                                if (sdr.HasRows)
                                                {
                                                    ...
                                                    if (this.CheckBox_保存验证.Checked == true)//判斷是否選擇了記住用戶名和密碼的複選框
                                                    {
                                                        ...
                                                        if (Request.Cookies["PPusernameBackstageL2"] != null && Request.Cookies["PPusernameBackstageL2"] != null)
                                                        {
                                                            Response.Cookies["PPusernameBackstageL2"].Expires = System.DateTime.Now.AddSeconds(-1);//Expires過期時間
                                                            Response.Cookies["PPusernameBackstageL2"].Expires = System.DateTime.Now.AddSeconds(-1);
                                                        }
                                                        //向客戶端瀏覽器加入Cookie (用戶名和密碼 最好是使用MD5加密)
                                                        HttpCookie hcUserName1 = new HttpCookie("PPusernameBackstageL2");
                                                        ...
                                                        string 生成编号 = "BLLL2" + DateTime.Now.ToString("yyyyMMddHHmmss") + Convert.ToString(ClassLibrary1.ClassHelpMe.GenerateRandomCode(1, 1000, 9999));
                                                        //---操作名称
                                                        string 操作 = "登入后台账户";
                                                        //---获取登入IP地址
                                                        string ipaddress;
                                                        ...
                                                        Response.Redirect("~/WebsiteBackstage/L2/ControlCenter/管理控制台.aspx");  //刷新頁面
                                                    }
                                                    else
                                                    {
