...
namespace web1.WebsiteBackstage.L2.ManagementOrder
{
    public partial class 商户充值记录 : System.Web.UI.Page
    {
        ...
        private string 查询组合商户()
        {
            string Cookie_UserName = ClassLibrary1.ClassAccount.检查管理L2端cookie2();

            string dasdsa = " 商户ID in (select 商户ID from table_商户账号 where 所属管理L2 = '" + Cookie_UserName + "')  and ";
            return dasdsa;
        }

        public string 查看勾选了哪些()
        {
            return 查询组合商户() + 获得类型() + 获得筛选关键字() + 获得区间金额() + 获得时间();
        }
        ...
        private void BindGrid(string 时间导入绑定)
        {
            ...
            using (MySqlConnection connC = new MySqlConnection(ClassLibrary1.ClassDataControl.conStr1))
            {
                ...
                MySqlCommand cmd1 = new MySqlCommand("select COUNT(订单号) from table_商户明细充值 where 充值类型='充值提款余额' and 状态='成功' and " + 时间导入绑定 + " ", connC);
                ...
                MySqlCommand cmd2 = new MySqlCommand("select COUNT(订单号) from table_商户明细充值 where 充值类型='充值提款手续费' and 状态='成功' and " + 时间导入绑定 + " ", connC);
                ...
                MySqlCommand cmd3 = new MySqlCommand("select sum(充值金额) from table_商户明细充值 where 充值类型='充值提款余额' and 状态='成功' and " + 时间导入绑定 + " ", connC);
                ...
                MySqlCommand cmd4 = new MySqlCommand("select sum(充值金额) from table_商户明细充值 where 充值类型='充值提款手续费' and 状态='成功' and " + 时间导入绑定 + " ", connC);
            ...
            string strQuery = "select 订单号,商户ID,商户银行卡卡号,商户充值目标卡号,充值类型,充值金额,备注后台,状态,时间创建 from table_商户明细充值 where " + 时间导入绑定 + " order by 时间创建 desc  LIMIT " + 分页() + " ";
