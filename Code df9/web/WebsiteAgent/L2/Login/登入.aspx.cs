...
namespace web1.WebsiteAgent.L2.Login
{
        ...
        protected void Button_登入_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                    ...
                    TextBox_回答.Text.Length < 10)
                {
                    ...
                    {
                        using (MySqlCommand cmd11 = new MySqlCommand("SELECT 代理ID,keyga,登入错误累计,状态 FROM table_代理账号等级2 WHERE 代理ID=@代理ID", con11))
                        {
                                    ...
                                    if(状态=="启用")
                                    {
                                        if (登入错误累计<6)
                                        {
                                            ...
                                            if (result)
                                            {
                                                ...
                                                conn.Open();
                                                MySqlCommand cmd = new MySqlCommand("select * from table_代理账号等级2 where 代理ID=@代理ID and 代理密码=@代理密码 ", conn);
                                                ...
                                                if (sdr.HasRows)
                                                {
                                                    ...
                                                    if (this.CheckBox_保存验证.Checked == true)//判斷是否選擇了記住用戶名和密碼的複選框
                                                    {
                                                        ...
                                                        if (Request.Cookies["PPusernameAgentL2"] != null && Request.Cookies["PPusernameAgentL2"] != null)
                                                        {
                                                            Response.Cookies["PPusernameAgentL2"].Expires = System.DateTime.Now.AddSeconds(-1);//Expires過期時間
                                                            Response.Cookies["PPusernameAgentL2"].Expires = System.DateTime.Now.AddSeconds(-1);
                                                        }
                                                        //向客戶端瀏覽器加入Cookie (用戶名和密碼 最好是使用MD5加密)
                                                        HttpCookie hcUserName1 = new HttpCookie("PPusernameAgentL2");
                                                        ...
                                                        //HttpCookie hcPassword1 = new HttpCookie("PPusernameAgentL2");
                                                        ...
                                                        string 生成编号 = "LLAL2" + DateTime.Now.ToString("yyyyMMddHHmmss") + Convert.ToString(ClassLibrary1.ClassHelpMe.GenerateRandomCode(1, 1000, 9999));
                                                        //---操作名称
                                                        string 操作 = "登入代理账户";
                                                        //---获取登入IP地址
                                                        string ipaddress;
                                                        ...
                                                        Response.Redirect("~/WebsiteAgent/L2/Overview/代理首页L2.aspx");  //刷新頁面
                                                    }
                                                    else
                                                    {
