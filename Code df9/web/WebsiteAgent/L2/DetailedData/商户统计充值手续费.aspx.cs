...
namespace web1.WebsiteAgent.L2.DetailedData
{
    public partial class 商户统计充值手续费 : System.Web.UI.Page
    {
        ...
        private void BindGrid(string 时间导入绑定)
        {
            ...
            string strQuery = "select A.商户ID,max(B.商户名称),count(A.商户ID),sum(A.充值金额),sum(A.产生手续费) FROM table_商户明细充值 A left join table_商户账号 B ON A.商户ID = B.商户ID where A.充值类型='充值提款手续费' and A.状态='成功' and  A.商户ID in (select 商户ID from table_商户账号 where 所属代理L2='" + Cookie_UserName + "')  and " + 时间导入绑定 + " group by A.商户ID  LIMIT " + 分页() + " ";
