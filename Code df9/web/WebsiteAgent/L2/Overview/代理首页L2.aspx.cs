...
namespace web1.WebsiteAgent.L2.Overview
{
    public partial class 代理首页L2 : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                ClassLibrary1.ClassAccount.检查代理L2端cookie();
            }
        ...
        {
            string Cookie_UserName = ClassLibrary1.ClassAccount.检查代理L2端cookie2();

            using (MySqlConnection connC = new MySqlConnection(ClassLibrary1.ClassDataControl.conStr1))
            {
                connC.Open();

                MySqlCommand cmd1 = new MySqlCommand("select COUNT(*) from table_商户明细提款 where 状态='成功' and  商户ID in (select 商户ID from table_商户账号 where 所属代理L2='" + Cookie_UserName + "')  and " + 这里填时间, connC);
                ...
                MySqlCommand cmd2 = new MySqlCommand("select sum(交易金额) from table_商户明细提款 where 状态='成功' and  商户ID in (select 商户ID from table_商户账号 where 所属代理L2='" + Cookie_UserName + "')  and " + 这里填时间, connC);
                ...
                MySqlCommand cmd3 = new MySqlCommand("select sum(手续费) from table_商户明细提款 where 状态='成功' and  商户ID in (select 商户ID from table_商户账号 where 所属代理L2='" + Cookie_UserName + "')  and " + 这里填时间, connC);
        ...
        private void 查询商户充值手续费(string 这里填时间)
        {
            ...
            {
                connC.Open();
                MySqlCommand cmd1 = new MySqlCommand("select COUNT(*) from table_商户明细充值 where 充值类型='充值提款手续费' and 状态='成功' and  商户ID in (select 商户ID from table_商户账号 where 所属代理L2='" + Cookie_UserName + "')  and " + 这里填时间, connC);
                object obj1 = cmd1.ExecuteScalar();
                if (obj1 != null)
                {
                ...
                MySqlCommand cmd2 = new MySqlCommand("select sum(充值金额) from table_商户明细充值 where 充值类型='充值提款手续费' and 状态='成功' and  商户ID in (select 商户ID from table_商户账号 where 所属代理L2='" + Cookie_UserName + "')  and " + 这里填时间, connC);
                object obj2 = cmd2.ExecuteScalar();
                if (obj2 != null)
                {
                ...
                //MySqlCommand cmd3 = new MySqlCommand("select sum(产生手续费) from table_商户明细充值 where 充值类型='充值提款手续费' and 状态='成功' and  商户ID in (select 商户ID from table_商户账号 where 所属代理L2='" + Cookie_UserName + "')  and " + 这里填时间, connC);
                //object obj3 = cmd3.ExecuteScalar();
                //if (obj3 != null)
                //{
        ...
        private void 查询商户充值余额(string 这里填时间)
        {
            ...
            {
                connC.Open();
                MySqlCommand cmd1 = new MySqlCommand("select COUNT(*) from table_商户明细充值 where 充值类型='充值提款余额' and 状态='成功' and  商户ID in (select 商户ID from table_商户账号 where 所属代理L2='" + Cookie_UserName + "')  and " + 这里填时间, connC);
                object obj1 = cmd1.ExecuteScalar();
                if (obj1 != null)
                {
                ...
                MySqlCommand cmd2 = new MySqlCommand("select sum(充值金额) from table_商户明细充值 where 充值类型='充值提款余额' and 状态='成功' and  商户ID in (select 商户ID from table_商户账号 where 所属代理L2='" + Cookie_UserName + "')  and " + 这里填时间, connC);
                object obj2 = cmd2.ExecuteScalar();
                if (obj2 != null)
                {
                ...
                MySqlCommand cmd3 = new MySqlCommand("select sum(产生手续费) from table_商户明细充值 where 充值类型='充值提款余额' and 状态='成功' and  商户ID in (select 商户ID from table_商户账号 where 所属代理L2='" + Cookie_UserName + "')  and " + 这里填时间, connC);
                object obj3 = cmd3.ExecuteScalar();
                if (obj3 != null)
                {
