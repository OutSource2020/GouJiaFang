...
{
    public partial class 商户统计提款笔数 : System.Web.UI.Page
    {
        ...
        private void BindGrid(string 时间导入绑定)
        {
            ...
            string strQuery = "select A.商户ID,max(B.商户名称),count(A.手续费),sum(A.交易金额),sum(A.手续费) from table_商户明细提款 A left join table_商户账号 B ON A.商户ID = B.商户ID where  A.状态='成功' and  A.商户ID in (select 商户ID from table_商户账号 where 所属代理L1='" + Cookie_UserName + "')  and " + 时间导入绑定 + " group by A.商户ID  LIMIT " + 分页() + " ";
            ...
            using (MySqlConnection connC = new MySqlConnection(ClassLibrary1.ClassDataControl.conStr1))
            {
                ...
                MySqlCommand cmd1 = new MySqlCommand("select count(商户ID) from table_商户明细提款 where 商户ID in (select 商户ID from table_商户账号 where 所属代理L1='" + Cookie_UserName + "') and 状态='成功' and " + 时间导入绑定 + " order by 商户ID desc", connC);
                object obj1 = cmd1.ExecuteScalar();
                if (obj1 != null)
                {
                    Label_提款笔数.Text = obj1.ToString();
                }

                MySqlCommand cmd2 = new MySqlCommand("select sum(交易金额) from table_商户明细提款 where 商户ID in (select 商户ID from table_商户账号 where 所属代理L1='" + Cookie_UserName + "') and 状态='成功' and " + 时间导入绑定 + " order by 商户ID desc", connC);
                object obj2 = cmd2.ExecuteScalar();
                if (obj2 != null)
                {
                    Label_提款金额.Text = obj2.ToString();
                }

                MySqlCommand cmd3 = new MySqlCommand("select sum(手续费) from table_商户明细提款 where 商户ID in (select 商户ID from table_商户账号 where 所属代理L1='" + Cookie_UserName + "') and 状态='成功' and " + 时间导入绑定 + " order by 商户ID desc", connC);
