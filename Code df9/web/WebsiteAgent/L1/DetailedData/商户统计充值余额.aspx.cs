...
{
    public partial class 商户统计充值余额 : System.Web.UI.Page
    {
        ...
        private void BindGrid(string 时间导入绑定)
        {
            ...
            string strQuery = "select A.商户ID,max(B.商户名称),count(A.商户ID),sum(A.充值金额),sum(A.产生手续费) from table_商户明细充值 A left join table_商户账号 B ON A.商户ID = B.商户ID where A.充值类型='充值提款余额' and A.状态='成功' and  A.商户ID in (select 商户ID from table_商户账号 where 所属代理L1='" + Cookie_UserName + "')  and " + 时间导入绑定 + " group by A.商户ID  LIMIT " + 分页() + " ";
            ...
            using (MySqlConnection connC = new MySqlConnection(ClassLibrary1.ClassDataControl.conStr1))
            {
                ...
                MySqlCommand cmd1 = new MySqlCommand("select count(商户ID) from table_商户明细充值 where 商户ID in (select 商户ID from table_商户账号 where 所属代理L1='" + Cookie_UserName + "') and 充值类型='充值提款余额' and 状态='成功' and " + 时间导入绑定 + " order by 商户ID desc", connC);
                ...
                MySqlCommand cmd2 = new MySqlCommand("select sum(充值金额) from table_商户明细充值 where 商户ID in (select 商户ID from table_商户账号 where 所属代理L1='" + Cookie_UserName + "') and 充值类型='充值提款余额' and 状态='成功' and " + 时间导入绑定 + " order by 商户ID desc", connC);
                ...
                MySqlCommand cmd3 = new MySqlCommand("select sum(产生手续费) from table_商户明细充值 where 商户ID in (select 商户ID from table_商户账号 where 所属代理L1='" + Cookie_UserName + "') and 充值类型='充值提款余额' and 状态='成功' and " + 时间导入绑定 + " order by 商户ID desc", connC);
